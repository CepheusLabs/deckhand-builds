...
inline std::string create_metadata_scan_json(int cmd, nlohmann::json params) {
    response_type_id = cmd;
    nlohmann::json api;
    api["jsonrpc"] = "2.0";
    api["method"] = "server.files.metascan";
    api["params"] = params;
    api["id"] = 3546;//method2id[cmd];
    return api.dump();
}
...
std::string json_gcode_metascan(std::string filename) {
    nlohmann::json params;
    params["filename"] = filename;
    return create_metadata_scan_json(0x52, params);
}

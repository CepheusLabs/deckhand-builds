...
// 设置控件透明度
void send_cmd_aph(int fd, std::string obj, std::string aph)
{
    std::string cmd = obj + ".aph=" + aph + "\xff\xff\xff";
    write(fd, cmd.data(), cmd.length());
}
...
// 往曲线控件添加数据
void send_cmd_language(int fd, std::string val)
{
    std::string cmd = "lang=" + val + "\xff\xff\xff";
    MKSLOG_YELLOW("%s", cmd.data());
    write(fd, cmd.data(), cmd.length());
}
...
// 切换外部图片路径 (仅X3,X5系列支持)
void send_cmd_path(int fd, std::string obj, std::string path)
{
    std::string cmd = obj + ".path=" + "\"" + path + "\"" + "\xff\xff\xff";
    write(fd, cmd.data(), cmd.length());
}
...
// 修改按钮上下效果图
void send_cmd_picc_picc2(int fd, std::string obj, std::string picc, std::string picc2)
{
    send_cmd_picc(fd, obj, picc);
    send_cmd_picc2(fd, obj, picc2);
}
...
void send_cmd_cp_write(int fd, std::string data)
{
...
{
    char *cmd = (char *)"baud=921600\xff\xff\xff";
    write(fd, cmd, 15);
}

...
class PCommandsMixin:
    ...
    def Cmds_CmdP114(self, gcmd):
        ...
        # # parameter isS，i.e.read multi-color mainboard simple parameter
        # if "S" in params:
        ...
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]noeffect response should command '%s'" % (gcmd.get_commandline(),))
        #     #     return
        ...
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==waitmachinestep section==%d" % MC_STANDBY)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_PREPARTION:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==prepare material stoprely step section==%d" % MC_STANDBY)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_CHANGING_P1:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==filament changestep section1==%d" % MC_CHANGING_P1)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_CHANGING_P2:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==filament changestep section2==%d" % MC_CHANGING_P2)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_FORCE_FEED:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==filament changestep section force refill==%d" % MC_FORCE_FEED)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_PRINTING:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==printstep section refill==%d" % MC_PRINTING)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_ROLLBACK:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==completecomplete unload filament==%d" % MC_ROLLBACK)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_PARKBACK:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==unload filament to park position==%d" % MC_PARKBACK)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_PARKALL:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==all unload filament to park position==%d" % MC_PARKALL)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_CLEANING:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==all filamentmaterial clear==%d" % MC_CLEANING)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_ERR_TIMEOUT:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==timeout outwrong state==%d" % MC_ERR_TIMEOUT)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_ERR_RUNOUT:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==runout outwrong state==%d" % MC_ERR_RUNOUT)
        #     # if int(Lo_AMSDeviceStateInfo.field.mc_state) is MC_ERR_BLOCKUP:
        #     #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]current statestate==clog outwrong state==%d" % MC_ERR_BLOCKUP)
        #     # self.G_AMS1DeviceState["ma_state"] = Lo_AMSSimpleState["ma_state"] = Lo_AMSDeviceStateInfo.field.ma_state
        #     # # response should datajsonrotate/switchchange
        ...
        #self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]P114block serial portcatch/connect receive: %s" % Lo_AMSDeviceStateRspInfo)
        ...
        #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]AMSnot yetresponse should，please checkAMS '%s'" % (gcmd.get_commandline(),))
        #     #lancaigang240510：
        #     self.G_PhrozenFluiddRespondInfo("+P114:-1")
        #     #self.G_P114RunFlag=0
        ...
        # self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]AMShasresponse should")
        ...
        # #lancaigang240524：not use，assign-1
        ...
        # #self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]entry position sensor state(bitposition/bit)==%d" % Lo_AMSDeviceStateInfo.field.entry_state)
        # self.G_AMS1DeviceState["park_state"] = Lo_AMSDetailState["park_state"] = Lo_AMSDeviceStateInfo.field.park_state
        # #self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP114]park position sensor state(bitposition/bit)==%d" % Lo_AMSDeviceStateInfo.field.park_state)
...
####################################
    ...
    def Cmds_CmdP28(self, gcmd):
        ...
        # #lancaigang250807:thousand ten thousand cannot clear pause state，way during printingthreeparty sendingP28
        ...
        # #lancaigang231122：usettyUSB0before，need closebackgroundIAPriselevel processorder/sequencehdl_zigbee_gateway
        ...
        if self.G_SerialPort1Obj is not None:
            ...
            if self.G_SerialPort1Obj.is_open:
                ...
                # self.G_PhrozenFluiddRespondInfo("V-H%s-I%s-F%s" % (HW_VERSION,IMAGE_VERSION,FW_VERSION))
                # #lancaigang240104：return to touchscreen
                ...
                #     self.G_PhrozenFluiddRespondInfo("send command: M0mode")
                ...
                if self.G_SerialPort1OpenFlag == True:
                    ...
                    # if len(Lo_AMSDeviceStateRspInfo) != sizeof(AMSDetailInfoBytes):
                    #     self.G_PhrozenFluiddRespondInfo("AMS1not yetresponse should，please checkAMS '%s'" % (gcmd.get_commandline(),))
                    #     #lancaigang240412:AMSmulti-color flag
                    #     self.G_AMSDevice1IfNormal=False
                    # else:
                    #     self.G_PhrozenFluiddRespondInfo("AMS1connect success '%s'" % (gcmd.get_commandline(),))
                ...
                # if self.G_SerialPort2Obj is not None:
                #     if self.G_SerialPort2Obj.is_open:
                #         self.G_PhrozenFluiddRespondInfo("serial port2is open，continue")
                #     else:
                #         self.G_PhrozenFluiddRespondInfo("V-H%s-I%s-F%s" % (HW_VERSION,IMAGE_VERSION,FW_VERSION))
                #         self.G_PhrozenFluiddRespondInfo("+AMSCONNECT:0")
                #         #return
        ...
        if self.G_SerialPort2Obj is not None:
            if self.G_SerialPort2Obj.is_open:
                ...
                if self.G_SerialPort2OpenFlag == True:
                    ...
                    # if len(Lo_AMSDeviceStateRspInfo) != sizeof(AMSDetailInfoBytes):
                    #     self.G_PhrozenFluiddRespondInfo("AMS2not yetresponse should，please checkAMS '%s'" % (gcmd.get_commandline(),))
                    #     self.G_AMSDevice2IfNormal=False
                    # else:
                    #     self.G_PhrozenFluiddRespondInfo("AMS2connect success '%s'" % (gcmd.get_commandline(),))
        ...
        # #ifttyUSB0still nothit/open open/enable and directly send，klipprsystem/relation system error report
            ...
            if self.G_SerialPort1Obj.is_open:
                ...
                if self.G_ToolheadIfHaveFilaFlag:
                    ...
                    if self.G_AMSDeviceWorkMode == AMS_WORK_MODE_MC:
                        ...
                        # self.G_PhrozenFluiddRespondInfo("[(cmds.python)]external macro-PRZ_CZ；cut filament before，first wipe nozzle")
                        # command_string = """
                        # PRZ_CZ
                        # """
                        # self.G_PhrozenGCode.run_script_from_command(command_string)
                    ...
                    #if self.G_AMSDeviceWorkMode == AMS_WORK_MODE_MA:
                    #    #lancaigang231202：home/reset cut filament and not retract
                    ...
                    #time.sleep(20)
                    #raise gcmd.error("[(cmds.python)Cmds_CmdP28]AMSmulti-color connect failure")
                ...
                #raise gcmd.error("AMS1multi-color connect failure")
        except:
            self.G_PhrozenFluiddRespondInfo("serial port11time(s)hit/open open/enable failure")
        ...
        try:
            ...
            if self.G_SerialPort2Obj.is_open:
                ...
                #raise gcmd.error("AMS2multi-color connect failure")
        except:
            self.G_PhrozenFluiddRespondInfo("serial port21time(s)hit/open open/enable failure")
            #self.G_PhrozenFluiddRespondInfo("+AMSCONNECT:2")
            self.G_PhrozenFluiddRespondInfo("+AMSERROR:2")
    ...
    # P9 
    # X[x_pos] x_pos:wait area base pointXcoordinate
    # Y[y_pos] y_pos:wait area base pointYcoordinate
    # W[width] width:wait area width 
    # H[height] height:wait area height 
    # D[0/5] D?:protect hold original appearance
    ...
    def Cmds_CmdP9(self, gcmd):
        ...
        if "D" in params:
            ...
            #     raise gcmd.error("noeffect wait area zone set，Dparameter must is[0/1] '%s'" % (gcmd.get_commandline(),))
        ...
        # Python list(List)
        # sequence isPythonin most basicthis datastructure。sequence in eachelement element alldivide match/config anumbercharacter - it position，orindex，aindex is0，twoindex is1，depend thisclass/type push。
        # Pythonhas6sequence insideset type，but mostoften/normalsee is list and tuple。
        # sequence all can performexecute/row operationpackage include index，cutchip/piece，add，multiply，checkbecomemember。
        # this outside，Pythonbuilt-indeterminedsequence length and determine mostlarge and mostsmall element element method。
        # list is mostoften/normal use Pythondata type，it canas is abrackets inside commaseparated values outFound.
...
####################################
    ...
    def Cmds_CmdP0M3P8FA(self, AMSNum,gcmd):
        # if not self.G_SerialPort1OpenFlag:
        #     self.G_PhrozenFluiddRespondInfo("AMSmulti-color not yet connect，please first sendP28")
        #     return
    ...
    def Cmds_CmdP8(self,gcmd):
        ...
        # #lancaigang231205：home/reset cut filament retract
        # self.Cmds_MoveToCutFilaAndNotRollback(gcmd)
        # self.G_PhrozenFluiddRespondInfo("allAMSfirst retract")
        # #lancaigang241030:
        # if self.G_SerialPort1OpenFlag == True:
        #     self.Cmds_AMSSerial1Send("AP")
        #     logging.info("serial port1send command：AP")
        ...
        if "A" in params:
            ...
            if action == 2:
                ...
                if self.G_ToolheadIfHaveFilaFlag:
                    ...
                    # self.G_PhrozenFluiddRespondInfo("AMSold channel retract some distance first: G%d" % self.G_ChangeChannelTimeoutOldChan)
                    #lancaigang241030:
                    if self.G_SerialPort1OpenFlag == True:
                        self.Cmds_AMSSerial1Send("AP")
                        logging.info("serial port1send command：AP")
                    ...
                    # self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]external macro-PG101")
                    # command_string = """
                    #     PG101
                    #     """
                    # self.G_PhrozenGCode.run_script_from_command(command_string)
                    # self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]external macro-tospecified wait area position wait purge；command_string='%s'" % command_string)
                    # self.IfDoPG102Flag=True
    ...
    def Cmds_CmdP1(self, gcmd):
        ...
        if self.G_AMSDevice1IfNormal==False and self.G_AMSDevice2IfNormal==False:
            ...
            if "I" in params:
                ...
                #                 """ %(int(params["I"]),)
                # self.G_PhrozenGCode.run_script_from_command(command_string)
                # self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]GCODEcommand: %s" % command_string)
        ...
        # #lancaigang240705：ifP114withoutAMS，then directly extrude
        ...
        #     # =====P1 I[n]；manual extrude whenstm32need refill；====="I?"；
        #     if "I" in params:
        #         self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]AMSmulti-colorP114not yet connect，directly extrude")
        #         #lancaigang240524：use inUIUXdynamic interface
        #         self.G_PhrozenFluiddRespondInfo("+P1In:0,%d" % self.G_ChangeChannelTimeoutNewChan)
        #         if not int(params["I"]) in range(-1000, 1000):
        #             raise gcmd.error("noeffect parameter command;cmd '%s', manual extrude past/overlong" % (gcmd.get_commandline()))
        ...
        #         #lancaigang240705：encapsulated function
        #         self.Cmds_P1InExtrudeManualIn(int(params["I"]))
        ...
        #if self.G_ToolheadIfHaveFilaFlag:
        #    gcmd.respond_info("[(cmds.python)Cmds_CmdP1]detect to toolhead has filament，firstcut runoutmaterial，againretract out all thread to park position")
        #    gcmd.respond_info("[(cmds.python)Cmds_CmdP1]toolhead cut filament")
        #    self.Cmds_MoveToCutFilaAction(gcmd)
        #    self.Cmds_AMSSerial1Send("AP")
        ...
        if "S" in params:
            ...
            # #lancaigang250429：
            # if self.G_AMSDeviceWorkMode==AMS_WORK_MODE_FILA_RUNOUT:
            #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdT0]single-color mode，not handleT0")
            #     return
            # #lancaigang250514：
            # if self.G_AMSDeviceWorkMode==AMS_WORK_MODE_MA :
            #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdT0]single-color refill mode，not handleT0")
            #     return
        ...
        if "T" in params:
            ...
            #lancaigang240113：manual commandflag/mark
            self.ManualCmdFlag=True
            # #lancaigang240529：refill process in not pause
            # self.Cmds_AMSSerial1Send("M0")
            # self.G_PhrozenFluiddRespondInfo("send command=M0")
            ...
            # self.G_PhrozenFluiddRespondInfo("external macro-PG109；cut filament after，first heat and purge/spitresidual toolhead filament")
            # self.PG102Flag=True
            # logging.info("self.Flag=True")
            # command_string = """
            # PG109
        ...
        if "B" in params:
            ...
            # #lancaigang240529：refill process in not pause
            # self.Cmds_AMSSerial1Send("M0")
            # self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]send command=M0")
            # self.G_ProzenToolhead.dwell(1)
            ...
            # #self.Cmds_MoveToCutFilaAbsolutePositionNotReset(gcmd)
            # #lancaigang231205：home/reset cut filament retract
            # #lancaigang240328：home/reset cut filament not retract
            # self.Cmds_MoveToCutFilaAndNotRollback(gcmd)
            ...
            # self.G_PhrozenFluiddRespondInfo("[(cmds.python)]external macro-PG109；cut filament after，first purge/spitresidual toolhead filament")
            # self.PG102Flag=True
        ...
        if "E" in params:
            ...
            # #lancaigang240527：unknown mode，because need operation manual command，default letSTM32perform enterMCmulti-color mode
            # if self.G_AMSDeviceWorkMode == AMS_WORK_MODE_UNKNOW:
            #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]unknown mode，because need operation manual command，default letSTM32perform enterMCmulti-color mode")
            #     self.Cmds_AMSSerial1Send("MC")
            #     self.G_PhrozenFluiddRespondInfo("[(cmds.python)Cmds_CmdP1]send command：MC")
        ...
        if "M" in params:
            ...
            #lancaigang250511：ifstore atAMSmulti-color，then autostart use auto refill mode；P0 M3rotate/switchchange isP0 M2
                ...
                if self.G_AMSDevice1IfNormal==True or self.G_AMSDevice2IfNormal==True:
                    ...
                    # #lancaigang250511：if hasAMSmulti-color，single-color mode
                    # #P0 M3
                    # #P8
                    # #lancaigang250514：multi-colorhit/open single-color
                    # self.G_PhrozenFluiddRespondInfo("P8")
                    # #lancaigang241106：
                    # self.Cmds_CmdP8(gcmd)
